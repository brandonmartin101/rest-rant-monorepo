...
router.post('/', async (req, res) => {
	if (req.currentUser?.role !== 'admin') {
		return res.status(403).json({ message: 'You must be an admin to create a place' });
	}
	if (!req.body.pic) {
...
router.put('/:placeId', async (req, res) => {
	if (req.currentUser?.role !== 'admin') {
		return res.status(403).json({ message: 'You must be an admin to update a place' });
	}
	let placeId = Number(req.params.placeId);
	if (isNaN(placeId)) {
		res.status(404).json({ message: `Invalid id "${placeId}"` });
...
router.delete('/:placeId', async (req, res) => {
	if (req.currentUser?.role !== 'admin') {
		return res.status(403).json({ message: 'You must be an admin to delete a place' });
	}
	let placeId = Number(req.params.placeId);
	if (isNaN(placeId)) {
		res.status(404).json({ message: `Invalid id "${placeId}"` });
...
router.delete('/:placeId/comments/:commentId', async (req, res) => {
	...
	if (isNaN(placeId)) {
		...
		if (!comment) {
			...
			res.status(403).json({ message: `You do not have permission to delete this comment: ${comment.commentId}` });
		} else {
